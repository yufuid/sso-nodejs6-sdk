...
var YUFU_CONSUME_URL = 'https://idp.yufuid.com/sso/v1/consume';
...
function init(options) {
    ...
    var verifySignedJwt = function(token, onFinish) {
        ...
        if (jwt && jwt.iss != yufuIssuer) {
            throw new SDKError('Invalid issuer of signed token.');
        }
        return jwt;
    };
...
function renewKeys() {
    https.get(YUFU_KEY_SERVICE_URL, (res) => {
        var rawData = '';
        ...
        res.on('end', () => {
            ...
            try {
                if (res.statusCode !== 200) {
                    throw new KeyNotAvailableError(`Request Failed. Status Code: ${res.statusCode}`);
                }
